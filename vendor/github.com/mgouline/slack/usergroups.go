...
func (api *Client) GetUserGroups(includeUsers bool) ([]UserGroup, error) {
	return api.GetUserGroupsContext(context.Background(), includeUsers)
}

// GetUserGroupsContext returns a list of user groups for the team with a custom context
func (api *Client) GetUserGroupsContext(ctx context.Context, includeUsers bool) ([]UserGroup, error) {
	values := url.Values{
		"token":         {api.token},
